...
export default function UserOrderDetailPage() {
  ...
  async function load() {
    try {
      ...
      // Enrich order items with product data if images are missing
      if (ord?.items?.length) {
        const bare = ord.items.filter(item => !item.product?.main_image && !item.product?.images?.length && item.product_id);
        if (bare.length > 0) {
          const results = await Promise.allSettled(bare.map(item => productsApi.get(item.product_id)));
          ...
          ord.items = ord.items.map(item =>
            (!item.product?.main_image && map[item.product_id])
              ? { ...item, product: map[item.product_id] }
              : item
          );
        }
      }
        ...
        <div style={S.col}>
          ...
          <div style={S.card}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              ...
              <a href={`https://karabo.onrender.com/api/orders/${id}/invoice`} target="_blank" rel="noreferrer" className="abtn gbtn" style={S.ghostBtnFull}>
                📄 Download Invoice
              </a>
          ...
          <div style={{ ...S.card, background: "#F8FAFC" }}>
            <p style={{ fontSize: 13, color: "#64748B", textAlign: "center", lineHeight: 1.8, margin: 0 }}>
              ...
              <a href={`https://wa.me/919253258848?text=Hi%2C+I+need+help+with+Order+%23${order.id.slice(0,8).toUpperCase()}`} style={{ color: ACCENT, fontWeight: 600 }}>Chat on WhatsApp</a>
              {" · "}
