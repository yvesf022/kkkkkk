...
export default function StorePage() {
  ...
  const loadProducts = useCallback(async (pg = 1, append = false) => {
    ...
    try {
      ...
      // Sort mapping
      const sortMap: Record<SortOption, Record<string, any>> = {
      ...
      Object.assign(params, sortMap[sort]);

      let results: ProductListItem[];
      let totalCount: number;

      if (searchQuery.trim()) {
        const res = await searchApi.search({ q: searchQuery, ...params }) as any;
      ...
      <div style={{
        ...
        <div style={{ maxWidth: 1440, margin: "0 auto", position: "relative", zIndex: 1 }}>
          ...
          <p style={{ color: "rgba(255,255,255,0.65)", fontSize: 15, margin: 0, fontWeight: 300 }}>
            {total > 0 ? `${total.toLocaleString()} products${selectedCategory ? ` in ${selectedCategory}` : ""}` : "Discover our full collection"}
          </p>
        </div>
      </div>
      ...
      <div style={{ maxWidth: 1440, margin: "0 auto", padding: "0 clamp(16px, 4vw, 64px)" }}>
        ...
        <div style={{
          ...
          <div ref={searchRef} style={{ position: "relative", flex: "1 1 260px", maxWidth: 420 }}>
            <div style={{ position: "relative" }}>
              ...
              />
              {searchInput && (
                <button onClick={() => { setSearchInput(""); setSearchQuery(""); setSuggestions([]); }} style={{ position: "absolute", right: 12, top: "50%", transform: "translateY(-50%)", background: "none", border: "none", cursor: "pointer", color: "#94a3b8", fontSize: 16 }}>✕</button>
              )}
            </div>
          ...
          >
            ⚙ Filters {hasActiveFilters && <span style={{ background: "#0033a0", color: "#fff", borderRadius: 99, padding: "1px 7px", fontSize: 11 }}>{[selectedCategory, selectedBrand, priceMin || priceMax, inStockOnly].filter(Boolean).length}</span>}
          </button>
          ...
          <div style={{ display: "flex", marginLeft: "auto", gap: 4 }}>
            {(["grid", "list"] as ViewMode[]).map((m) => (
              <button
                ...
                style={{ padding: "9px 12px", borderRadius: 9, border: `1.5px solid ${viewMode === m ? "#0f172a" : "#e2e0db"}`, background: viewMode === m ? "#0f172a" : "#fff", color: viewMode === m ? "#fff" : "#64748b", cursor: "pointer", fontSize: 14, transition: "all 0.15s" }}
              >
                {m === "grid" ? "⊞" : "☰"}
              </button>
            ))}
          ...
          {/* Results count */}
          <span style={{ fontSize: 13, color: "#94a3b8", flexShrink: 0, display: "none" }}>
            ...
            <div>
              ...
              <select value={selectedCategory} onChange={(e) => setSelectedCategory(e.target.value)} style={filterSelect}>
                ...
                {categories.map((c: any) => (
                  <option key={c.id ?? c.slug} value={c.slug ?? c.name}>{c.name} {c.product_count ? `(${c.product_count})` : ""}</option>
                ))}
              </select>
            </div>
              ...
              <label style={{ display: "flex", gap: 10, alignItems: "center", cursor: "pointer" }}>
                <div
                  ...
                  style={{
                    ...
                    background: inStockOnly ? "#0033a0" : "#e2e0db",
                    position: "relative", transition: "background 0.2s", cursor: "pointer",
                    flexShrink: 0,
                  }}
                >
                  <div style={{
                    position: "absolute", top: 3, left: inStockOnly ? "calc(100% - 21px)" : 3,
        ...
        {hasActiveFilters && (
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", padding: "8px 0 4px" }}>
            ...
            {(priceMin || priceMax) && <FilterChip label={`${priceMin ? formatCurrency(Number(priceMin)) : "0"} – ${priceMax ? formatCurrency(Number(priceMax)) : "∞"}`} onRemove={() => { setPriceMin(""); setPriceMax(""); }} />}
              ...
              {viewMode === "grid" ? (
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(min(100%, 220px), 1fr))", gap: "clamp(12px, 2vw, 22px)" }}>
                  {products.map((p, i) => (
                    <div
                      key={p.id}
                      style={{ animation: `fadeSlideUp 0.4s ease both`, animationDelay: `${Math.min(i, 15) * 40}ms` }}
                    >
                    ...
                    {Array.from({ length: Math.min(totalPages, 10) }, (_, i) => i + 1).map((pg) => (
                      <button
                        ...
                        style={{
                          ...
                          border: `1.5px solid ${pg === page ? "#0033a0" : "#e2e0db"}`,
                          background: pg === page ? "#0033a0" : "#fff",
  ...
  return (
    ...
    >
      <div style={{ width: 80, height: 80, borderRadius: 12, background: "#f1f0ee", overflow: "hidden", flexShrink: 0 }}>
        {product.main_image ? <img src={product.main_image} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : <div style={{ width: "100%", height: "100%", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 28 }}>📦</div>}
      </div>
